...
**Status:** Variant A confirmed as AquaClean Alba. iPhone BLE captures analyzed — **encrypted application-layer protocol confirmed**. Alba support is not feasible with the current bridge approach; binary analysis is legally questionable; a BLE MITM proxy is the remaining option but very high effort.
...
- How to decode the encrypted data payloads — requires binary analysis or a known encryption key
...
4–35          <32 bytes, differ between sessions>  Encrypted (session-specific ciphertext)
36–37         03 03                                Cleartext (fixed field delimiter)
38–39         <2 bytes, differ between sessions>   Session-specific ciphertext
...
**Per-session ciphertext is confirmed** — the encrypted bytes differ between captures. **Whether this is caused by a per-session key is a hypothesis** (see below).

### Handshake is fully deterministic — key is likely static (2026-04-24)

Comparing the complete handshake across both captures reveals that every frame is byte-for-byte identical:

| Frame | connect.txt | connect+actions.txt |
|---|---|---|
| Challenge (phone→device) | `00 04 2F F5 D9 00` | `00 04 2F F5 D9 00` ✓ |
| Response (device→phone) | `00 04 63 9D 51 00` | `00 04 63 9D 51 00` ✓ |
| Session parameters | `00 03 20 01 02 05 01 01 04 01 F8 1E 00` | identical ✓ |
| Session handle | `00 04 21 8B 30 00` | `00 04 21 8B 30 00` ✓ |

The challenge `2F F5 D9` is **not a random nonce** — it is fixed across sessions. If the encryption key were derived from the handshake, it would be **static**. Yet the ciphertext differs, which means one of:

1. **Static key + per-session IV** — the IV is generated by the app locally (not transmitted over BLE) and the device derives it via a shared counter or BLE pairing state. With AES-CBC this makes every ciphertext byte differ even for identical plaintext.
2. **Static key + changing plaintext** — the identification response embeds a session counter, timestamp, or nonce in the plaintext itself. With a block cipher, one changed byte early in the plaintext propagates to all subsequent blocks.
3. **Key derived from BLE pairing data** — the Long-Term Key (LTK) from BLE Secure Connections pairing provides a per-pairing static master key; a counter or IV derived from pairing state provides session differentiation.

In all three cases the **master key is likely static** — not regenerated each session. This makes the known-plaintext attack potentially more powerful: recovering the key once would decrypt all captures, not just one session.
...
The deterministic handshake means key derivation is not a barrier — the challenge and response are fixed and fully known. The unknown is the cipher mode and whether an IV is involved:

- **If XOR or AES-CTR with static IV:** known plaintext at a given offset directly yields those keystream bytes. Same keystream applies to all sessions → one known-plaintext pair decrypts all captures.
- **If AES-CBC with per-session IV:** known plaintext identifies which blocks are correct but does not directly reveal the IV. However, XOR-ing ciphertexts from two sessions at positions where the plaintext is known to be identical would isolate the IV contribution.
- **Distinguishing the two:** if `C1 XOR C2 = 0` at positions where the plaintext is identical (e.g. `"AquaClean Alba"`), the cipher is XOR/CTR with static key and static IV. If `C1 XOR C2 ≠ 0` at all positions, either CBC with changing IV or the plaintext itself changes.

**Practical requirement:** the SAP number and firmware version of the Alba device (visible in the Geberit Home app under device info). With these, the known-plaintext offset search can be attempted on the existing captures, and the XOR test run to distinguish cipher modes.
...
The application-layer key is likely either hardcoded in the Geberit Home app binary or derived during the `00 04 2F F5 D9 00` ↔ `00 04 63 9D 51 00` challenge-response handshake — both of which require binary analysis to recover.
