...
Moved 2026-07-20 to its own document: `docs/developer/application-layer-ble-relay-requirements.md`
(`RELAY-NNN` IDs) — the relay is a distinct component from this mock-service, even though it
reuses this mock's peripheral GATT server and Arendi crypto code, and deserved its own
requirements document rather than being folded into this one. See that document for the full
content (originally drafted here as REQ-061 through REQ-068 + REQ-ISS-001, now `RELAY-001`
through `RELAY-008` + `RELAY-ISS-001`). REQ-052 below is still referenced from there for the
same "related but distinct" reason noted at the time.
...
*(none logged yet — REQ-ISS-001, the Remote Control PSK blocker, moved to
`docs/developer/application-layer-ble-relay-requirements.md` as `RELAY-ISS-001` on 2026-07-20,
since it's specifically about the Alba-Hub relay, not this mock-service.)*
